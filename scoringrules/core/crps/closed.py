...
def lognormal(mulog: ArrayLike, sigmalog: ArrayLike, obs: ArrayLike, backend: str | None = None) -> Array:
...
def logistic(mu: ArrayLike, sigma: ArrayLike, obs: ArrayLike, backend: str | None = None) -> Array:
    """Compute the CRPS for the normal distribution."""
    B = backends.active if backend is None else backends[backend]
    mu, sigma, obs = map(B.asarray, (mu, sigma, obs))
    ω = (obs - mu) / sigma
