...
def energy_score(
    ...
    E_1 = B.sum(err_norm, -2) / M  # (...)

    spread_norm = B.norm(
        B.expand_dims(fcts, -2) - B.expand_dims(fcts, -1), -1
    )  # (... M M)
...
def owenergy_score(
    ...
    B = backends.active if backend is None else backends[backend]
    M = fcts.shape[-2]
    wbar = B.mean(fw, -1, keepdims=True)

    err_norm = B.norm(fcts - B.expand_dims(obs, -2), -1)  # (... M)
    E_1 = B.sum(err_norm * fw * B.expand_dims(ow, -1), -2) / (M * wbar)  # (...)
...
def vrenergy_score(
    ...
    rhobar = B.mean(B.norm(fcts, -1) * fw, -1)  # (...)
