...
def _variogram_score_gufunc(obs, fct, w, ens_w, p, out):
    M = fct.shape[-2]
    D = fct.shape[-1]
    out[0] = 0.0
    for i in range(D):
...
def _owvariogram_score_gufunc(obs, fct, w, ow, fw, ens_w, p, out):
    M = fct.shape[-2]
    D = fct.shape[-1]
...
def _vrvariogram_score_gufunc(obs, fct, w, ow, fw, ens_w, p, out):
    M = fct.shape[-2]
    D = fct.shape[-1]
