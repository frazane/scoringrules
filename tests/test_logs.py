...
def test_beta(backend):
    ...
    res = sr.logs_beta(obs, shape1, shape2, lower, upper, backend=backend)
    expected = -0.04344567
...
def test_binomial(backend):
    ...
    res = sr.logs_binomial(18, 30, 0.5, backend=backend)
    expected = 2.518839
    assert np.isclose(res, expected)
...
def test_gamma(backend):
    ...
    res = sr.logs_gamma(obs, shape, rate, backend=backend)
    assert np.isclose(res, expected)

    res = sr.logs_gamma(obs, shape, scale=1 / rate, backend=backend)
    assert np.isclose(res, expected)

    with pytest.raises(ValueError):
        sr.logs_gamma(obs, shape, rate, scale=1 / rate, backend=backend)
        return

    with pytest.raises(ValueError):
        sr.logs_gamma(obs, shape, backend=backend)
        return
...
def test_gev(backend):
    ...
    res = sr.logs_gev(obs, xi, mu, sigma, backend=backend)
...
def test_gpd(backend):
    ...
    res = sr.logs_gpd(obs, shape, location, scale, backend=backend)
...
def test_hypergeometric(backend):
    ...
    res = sr.logs_hypergeometric(5 * np.ones((2, 2)), 7, 13, 12, backend=backend)
    assert res.shape == (2, 2)

    res = sr.logs_hypergeometric(5, 7 * np.ones((2, 2)), 13, 12, backend=backend)
    assert res.shape == (2, 2)
...
def test_exponential2(backend):
    ...
    res0 = sr.logs_exponential(obs, 1 / scale, backend=backend)
    res = sr.logs_exponential2(obs, 0.0, scale, backend=backend)
    assert np.isclose(res, res0)
...
def test_laplace(backend):
    ...
    res = sr.logs_laplace(obs, location, backend=backend)
    expected = 3.793147
    assert np.isclose(res, expected)

    res = sr.logs_laplace(obs, location, scale, backend=backend)
    expected = 4.032231
    assert np.isclose(res, expected)
...
def test_negbinom(backend):
    ...
    res = sr.logs_negbinom(obs, n, mu=mu, backend=backend)
    expected = 3.247462
    assert np.isclose(res, expected)
...
def test_2pnormal(backend):
    ...
    res0 = sr.logs_normal(obs, location, scale, backend=backend)
    res = sr.logs_2pnormal(obs, scale, scale, location, backend=backend)
    assert np.isclose(res, res0)
    obs, mu, sigma = 17.1, 13.8, 3.3
    res = sr.logs_normal(obs, mu, sigma, backend=backend)
    expected = 2.612861
    assert np.isclose(res, expected)
...
def test_t(backend):
    ...
    obs, df = 0.7, 4.0
    res = sr.logs_t(obs, df, backend=backend)
    expected = 1.269725
    assert np.isclose(res, expected)
...
def test_tlogis(backend):
    ...
    res0 = sr.logs_logistic(obs, location, scale, backend=backend)
    res = sr.logs_tlogistic(obs, location, scale, backend=backend)
    assert np.isclose(res, res0)
...
def test_tnormal(backend):
    ...
    res0 = sr.logs_normal(obs, location, scale, backend=backend)
    res = sr.logs_tnormal(obs, location, scale, backend=backend)
    assert np.isclose(res, res0)
...
def test_tt(backend):
    ...
    # aligns with logs_t
    res0 = sr.logs_t(obs, df, location, scale, backend=backend)
    res = sr.logs_tt(obs, df, location, scale, backend=backend)
    assert np.isclose(res, res0)
